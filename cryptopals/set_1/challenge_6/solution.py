...
def avg_hamming_for_len(key_len):
    # calculate average hamming distance for a given number of pairs
    hamming_avg_range = range(4)
    curr_hamming = 0
    for i in hamming_avg_range:
        curr_hamming += hamming(in_bytes[key_len * i : key_len * (i + 1)],\
            in_bytes[key_len * (i + 1) : key_len * (i + 2)]) \
...
hamming_per_key_len = [(key_len, avg_hamming_for_len(key_len)) for key_len in key_len_range]
...
for idx in hamming_check_range:
    ...
    rearranged_bytes = [[in_bytes[i] for i in range(j,len(in_bytes),hamming_per_key_len[idx][0])] for j in range(hamming_per_key_len[idx][0])]
    # decrypt each group independently
    decrypted_bytes = [solve_single_char_xor(subset) for subset in rearranged_bytes]
